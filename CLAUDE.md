...
- всегда прочитывай файл линтера [eslint.config.mjs](eslint.config.mjs) и старайся сразу писать код соответствующий его ограничениям
- пофикшеный баг всегда должен быть покрыт тестами
...
- ты не можешь смягчать условия линтеров без моего прямого указания
...
2. `docker compose up --build -d` — пересобрать и запустить контейнер. Не останавливать — сервис должен работать постоянно.
3. `docker compose logs app --tail=50` — убедиться что в логах нет ошибок, приложение стартовало и watchers подписались.
...
5. Если правишь только конфиг/документацию без TS — хватит `npm run lint`.
...
- отдавай предпочтение популярным библиотекам, являющимся отраслевыми стандартами. обязательно согласовывай их использование со мной
- прежде чем писать интеграцию с любым внешним апи, сделай его смоук через курл, сразу запроси у меня необходимые токены для подключения если они нужны
