...
- В истории кошелька добавлены расширенные поля карточки транзакции: `txType`, `flowType/flowLabel`, `assetStandard`, `dex`, `pair`, `isError`, `counterpartyAddress`, `contractAddress`.
...
- После mute/unmute TMA обновляет:
  ...
  - `['wallets']`
  - `['tma-init']`
- Все переходы в TMA строятся только по `walletId` (без alias `id`), чтобы исключить ошибки `Некорректный id кошелька`.
- `Детали -> Показать ещё` больше не ломает карточку: для offset-страниц включён explorer fallback, а ошибка пагинации теперь локальная для секции истории.
- Для Solana/TRON история в TMA теперь explorer-first: если локальных событий мало, страница автоматически дополняется внешним источником, а `Показать ещё` стабильно открывает более старые транзакции.
- В секции истории вместо текста `Загрузка истории...` используется спиннер TelegramUI.
- Заголовок карточки в TMA больше не показывает `Кошелёк <id>` — теперь используется label кошелька (или нейтральный `Кошелёк`), чтобы экран выглядел чище.
- При открытии `Детали` TMA больше не делает повторный bootstrap авторизации на каждый рендер, из-за чего ушло ощущение полной перезагрузки экрана.
- В `AddWallet` добавлена явная навигация назад (`← Назад`) и интеграция Telegram BackButton.
- `Tx` в истории открывается через Telegram WebApp `openLink` (с fallback на браузер), поэтому ссылка стабильно работает в Mini App.
- Список кошельков в TMA больше не показывает `#id` рядом с названиями.
- Подсчёт `todayAlertCount` теперь корректно учитывает mixed-case адреса и chain-aware сопоставление событий.
- В карточке истории TMA отображаются rich-badges: `[txType] [direction] [flow] [assetStandard] [status]`.
...
- `/filters` - интерактивное меню фильтров (transfer/swap, режим include/exclude и чекбоксы DEX).
...
Callback-flow в Telegram:
- В `/filters` доступно inline-управление DEX без текстовых команд:
`include/exclude`, `toggle`, `reset`, `refresh`.
- В карточке `/wallet #id` кнопка `⚙️ Фильтры` вынесена в первый ряд действий.
- В `walletfilters` добавлен переход `↩️ К глобальным /filters`.
...
5. Устойчивый старт в Telegram WebView:
   - fallback чтение `initData`/`startapp` из query/hash,
   - явный экран ошибки авторизации вместо «черного экрана»,
   - базовые CSS-переменные Telegram темы, чтобы текст не терялся в dark mode.
6. Устойчивый dashboard-запуск:
   - при старте TMA JWT теперь всегда переинициализируется, если `initData` доступен,
   - при неуспешном `relogin()` очищаются протухшие токены,
   - экран dashboard показывает текст причины ошибки `/api/tma/init` и кнопку `Повторить`.
7. Базовый build фронтенда:
...
4. В `/start` добавлена заметная inline-кнопка запуска Mini App.
5. В основном keyboard первая кнопка теперь `🚀 Mini App` (если `TMA_BASE_URL` задан).
6. При старте приложения бот синхронизирует `Chat Menu Button` через Telegram Bot API на `web_app` URL с завершающим `/`, чтобы убрать запуск через устаревший `/tma` и потерю `initData`.
7. Для TMA auth добавлена защита от искажения `query_id` (`+` -> пробел) и безопасный парсинг launch-параметров без потери `+` в `tgWebAppData`.
...
Минимальная ручная проверка после релиза:

1. `/app` открывает TMA без черного экрана.
2. В списках `Dashboard` и `Wallets` у каждого кошелька виден chain-бейдж (`ETH/SOL/TRON`).
3. Кнопка `Открыть` в карточке кошелька ведет на валидный `/wallets/:walletId`.
4. После добавления кошелька открывается его detail-экран (без ошибки про id).
5. В detail-экране `🔕 Mute 24h` и `🔔 Unmute` работают и сразу обновляют состояние.
